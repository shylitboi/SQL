...
* 관계의 표기법
    ...
    2) 차수(Cardinality) : 각 엔터티에서 관계에 참여하는 수(1:1, 1:m, m:n)
        ```
        1. 식별자 관계에서 1:1 vs 1:M
        식별자 관계(Identifying Relationship)는 자식의 PK가 부모의 PK를 포함하는 관계.

        여기서 "1"과 "M"은 부모 PK 한 값이 자식 PK에서 몇 번 나타날 수 있는지를 뜻함.
        ...
        주민(주민등록번호 PK) ↔ 여권(주민등록번호 PK + 여권정보)
        → 주민 한 명은 여권 한 개만 가질 수 있음
        ...
        주문(주문번호 PK) ↔ 주문상세(주문번호 PK + 상품번호 PK)
        → 주문 한 건에 여러 상품 포함 가능
